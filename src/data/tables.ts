export const tableDataPart1 = [
  {
    study: 'HRV RCT',
    status: 'Published',
    n: '27',
    finding: '+43% global HRV / +166% (LF+HF)/HR',
    publication: 'Front Sports Act Living (2025)',
    implications: 'Provides peer-reviewed proof that the technology produces measurable physiological change after a single session. Establishes scientific credibility.'
  },
  {
    study: 'Case Study',
    status: 'Completed',
    n: '1',
    finding: '+21% HRV / -38% night awake duration / +15% sleep performance',
    publication: 'Internal (2025)',
    implications: 'Demonstrates that AV App can increase sleep performance and recovery during long air travel period reducing jetlag impact.'
  },
  {
    study: 'NIRS inflammation',
    status: 'Completed',
    n: '8',
    finding: '+10–15% SmO₂',
    publication: 'Internal (2025)',
    implications: 'Demonstrates improved local blood flow and tissue oxygenation. Supports claims around muscle recovery, reduced inflammation.'
  },
  {
    study: 'Lausanne Sport ’24',
    status: 'Completed',
    n: '22 sessions',
    finding: '+31.3% RMSSD (100% sessions improved)',
    publication: 'Manuscript in prep (2026)',
    implications: 'Shows real-world reliability in elite sport. Confirms that results are repeatable in competitive environments.'
  }
];

export const tableDataPart2 = [
  {
    study: 'Lausanne Sport ’25/26',
    status: 'Data complete',
    n: '19 AV / 10 control',
    finding: '+37% HRV / +12% sustained difference',
    publication: 'Manuscript in prep (2026)',
    implications: 'Demonstrates cumulative benefits over a competitive season. Indicates that regular use creates sustained recovery advantage.'
  },
  {
    study: 'Yverdon Sport ’23/24',
    status: 'Completed',
    n: '8 players',
    finding: '+25% HRV / +18% sleep / -25% fatigue',
    publication: 'Internal (2024)',
    implications: 'Validates season-long impact on recovery, sleep and match fatigue. Supports positioning in load management.'
  },
  {
    study: 'Long COVID',
    status: 'Ongoing',
    n: '20 patients',
    finding: 'Autonomic restoration, -46% symptom intensity',
    publication: 'Completion Q1 2026',
    implications: 'Expands applicability beyond sport into medical-adjacent markets. Opens pathway to wellbeing support in post-viral recovery.'
  },
  {
    study: 'Sleep (CLP)',
    status: 'Ongoing',
    n: '5/20 enrolled',
    finding: 'Early positive feedback',
    publication: 'Completion Q2 2026',
    implications: 'Positions AudioVitality in the premium longevity and executive health segment. Reinforces sleep optimisation.'
  },
  {
    study: 'Tinnitus',
    status: 'Ongoing',
    n: '191',
    finding: 'Real-world clinical dataset',
    publication: 'Ongoing',
    implications: 'Builds foundation for partner-led regulatory strategy (FDA 510(k)). Unlocks potential entry into specialist hearing markets.'
  },
  {
    study: 'Corporate Stress RCT',
    status: 'Planned',
    n: '40',
    finding: 'HRV, stress, sleep endpoints',
    publication: 'Launch Q1 2026',
    implications: 'Targets corporate wellness budgets with measurable ROI metrics. Supports enterprise sales and SaaS analytics positioning.'
  }
];

export const tableDataPart3 = [
  {
    study: 'Systemic sclerosis',
    status: 'Planned',
    n: '15–20 patients',
    finding: 'Microcirculation + inflammation',
    publication: 'Seeking partners/funding',
    implications: 'Explores autoimmune and inflammatory applications. Broadens long-term clinical relevance and strengthens medical credibility.'
  },
  {
    study: 'DOMS',
    status: 'Planned',
    n: '20 Active men',
    finding: 'DOMS + pain + oxygenation',
    publication: 'Launch Q1 2026',
    implications: 'Explore the effect of specific AV session on local inflammatory decrease after intensive effort.'
  },
  {
    study: 'LFVSS narrative review',
    status: 'Under publication',
    n: 'x',
    finding: 'Significant effects on heath',
    publication: 'Q1',
    implications: 'By precisely targeting mechanoreceptor populations, LFVSS offers a scalable solution for improving sports performance, managing stress-related autonomic dysfunction.'
  }
];
